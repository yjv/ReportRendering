...
class CsvEncoder
{
    ...
    public function __construct(array $options = array())
    {
        ...
        $this->optionsResolver
            ->setRequired(array('delimiter', 'enclosure', 'encloseAll', 'printNull', 'lineEnding'))
            ->setDefaults(array(
                'delimiter' => ',', 
                'enclosure' => '"',
                'encloseAll' => false, 
    ...
    protected function rowToCsv(array $row, array $options)
    {
        ...
        foreach ($row as $field) {
            ...
            if ($options['encloseAll']
                || preg_match("/(?:${delimiterEsc}|${enclosureEsc}|\s)/",
                    $field)) {

                $output[] = $enclosure
                    . str_replace($enclosure, '\\' . $enclosure, $field)
                    . $enclosure;
            } else {
                $output[] = $field;
            }
